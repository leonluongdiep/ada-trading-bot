# ==========================================
# 🤖 FUTURES-SPECIFIC ADAPTIVE SYSTEM (FIXED)
# ==========================================
...
# 3. Calculate DCA (Dollar Cost Average) size
...
# 4. Futures-specific TP/SL calculation
...
futures_adaptive_strategy <- function() {
  ...
  if (current_position$pnl < 0) {
    ...
    if (current_position$roe > -10) {  # Loss < 10%
      ...
      if (additional_size > 0) {
        ...
        cat("💡 Option 1: NO DCA (price too close to avg)\n")
      }
    }
  ...
  levels <- calculate_futures_tp_sl(current_position, portfolio)
...
#result <- futures_adaptive_strategy()

# Oder einzelne Funktionen testen:
analyze_futures_portfolio()
analyze_current_ada_position()
