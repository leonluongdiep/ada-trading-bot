...
place_tp_order <- function(symbol, side, size, trigger_price, symbol_info = NULL) {
  ...
  trigger_price_formatted <- if (!is.null(symbol_info)) format_price_precise(trigger_price, symbol_info) else sprintf("%.4f", round(trigger_price,4))
  trade_side <- if(side == "long") "sell" else "buy"
  body <- list(symbol=symbol, productType="USDT-FUTURES", marginMode="crossed", marginCoin="USDT",
               planType="profit_plan", triggerPrice=trigger_price_formatted,
               holdSide=side, side=trade_side, size=as.character(size),
               orderType="market", triggerType="fill_price")
  result <- bitget_request("/api/v2/mix/order/place-plan-order","POST", body)
  if (!is.null(result) && result$code=="00000") return(list(success=TRUE,order_id=result$data$orderId))
  cat("❌ TP Order failed:", if(!is.null(result)) result$msg else "Unknown", "\n")
  return(list(success=FALSE, error=if(!is.null(result)) result$msg else "Unknown"))
}
...
place_sl_order <- function(symbol, side, size, trigger_price, symbol_info = NULL) {
  ...
  trade_side <- if(side == "long") "sell" else "buy"
  body <- list(symbol=symbol, productType="USDT-FUTURES", marginMode="crossed", marginCoin="USDT",
               planType="loss_plan", triggerPrice=trigger_price_formatted,
               holdSide=side, side=trade_side, size=as.character(size),
               orderType="market", triggerType="fill_price")
  result <- bitget_request("/api/v2/mix/order/place-plan-order","POST", body)
  if (!is.null(result) && result$code=="00000") return(list(success=TRUE,order_id=result$data$orderId))
  cat("❌ SL Order failed:", if(!is.null(result)) result$msg else "Unknown", "\n")
  return(list(success=FALSE, error=if(!is.null(result)) result$msg else "Unknown"))
}
